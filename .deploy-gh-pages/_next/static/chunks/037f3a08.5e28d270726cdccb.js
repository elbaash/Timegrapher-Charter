...
`)})(o,c)}
  }`}})(o,n,s,i[0].dataType),{inputs:o})},ih=e=>tx({axis:e.axis})}),nP=L(()=>{ng(),nb(),im=(e,r,i="f32")=>{switch(e.activation){case"Relu":return`value = max(value, ${r}(0.0));`;case"Sigmoid":return`value = (${r}(1.0) / (${r}(1.0) + exp(-value)));`;case"Clip":return`value = clamp(value, ${r}(${i}(uniforms.clip_min)), ${r}(${i}(uniforms.clip_max)));`;case"HardSigmoid":return`value = max(${r}(0.0), min(${r}(1.0), ${i}(uniforms.alpha) * value + ${i}(uniforms.beta)));`;case"LeakyRelu":return`value = select(${i}(uniforms.alpha) * value, value, value >= ${r}(0.0));`;case"Tanh":return`let e2x = exp(-2.0 * abs(value));
              value = sign(value) * (1.0 - e2x) / (1.0 + e2x);
        `;case"":return"";default:throw Error(`Unsupported activation ${e.activation}`)}},ig=(e,r)=>{"Clip"===e.activation?r.push({type:1,data:e.clipMax},{type:1,data:e.clipMin}):"HardSigmoid"===e.activation?r.push({type:1,data:e.alpha},{type:1,data:e.beta}):"LeakyRelu"===e.activation&&r.push({type:1,data:e.alpha})},iy=(e,r)=>{"Clip"===e.activation?r.push({name:"clip_max",type:"f32"},{name:"clip_min",type:"f32"}):"HardSigmoid"===e.activation?r.push({name:"alpha",type:"f32"},{name:"beta",type:"f32"}):"LeakyRelu"===e.activation&&r.push({name:"alpha",type:"f32"})},i_=e=>{let r=e?.activation||"";if("HardSigmoid"===r){let[i,a]=e?.activation_params||[.2,.5];return{activation:r,alpha:i,beta:a}}if("Clip"===r){let[i,a]=e?.activation_params||[e7,e9];return{activation:r,clipMax:a,clipMin:i}}if("LeakyRelu"===r){let[i]=e?.activation_params||[.01];return{activation:r,alpha:i}}return{activation:r}}}),nU=L(()=>{ib=(e,r)=>{switch(e){case 1:return r;case 2:return`vec2<${r}>`;case 3:return`vec3<${r}>`;case 4:return`vec4<${r}>`;default:throw Error(`${e}-component is not supported.`)}},i$=e=>`
...
`}),nW=L(()=>{ng(),nb(),nT(),nP(),iw=(e,r,i,a,n)=>{let s=a-i;return`
...
`}return e})()}
  ...
  `}}}}),nL=L(()=>{ng(),nb(),nT(),nP(),nW(),nU(),ik=(e,r,i="f32",a,n=!1,s=32,o=!1,u=32)=>{let l=r[1]*e[1],d=r[0]*e[0],p=n?l:s,c=n?s:l,h=p/r[0],f=s/r[1];if(!((n&&4===h&&4===e[1]||!n&&(3===h||4===h))&&p%r[0]==0&&s%r[1]==0&&4===e[0]))throw Error(`If transposeA ${n} is true, innerElementSize ${h} and workPerThread[1] ${e[1]} must be 4.
